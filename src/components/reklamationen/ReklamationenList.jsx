...
import { LIST_GRID } from "../../lib/reklamationen/reklamationenFormat";
...
export default function ReklamationenList({
  ...
  return (
    <div className="pt-64 px-[80px]">
      {/* Header */}
      <div
        className={`grid ${LIST_GRID} px-4 text-left font-bold text-gray-300 border-b border-gray-500 pb-2 mb-6`}
      >
        <div className="flex items-center justify-center" title="Notiz">
          <Bookmark size={18} />
        </div>
        <div>lfd. Nr.</div>
        <div>Datum</div>
      ...
      {pagedData.map((rekla) => (
        ...
        >
          <div
            ...
            title={rekla?.has_notiz ? "Notiz vorhanden" : "Keine Notiz"}
          >
            <Bookmark
              size={18}
              className={rekla?.has_notiz ? "text-blue-600" : "text-gray-400"}
              fill={rekla?.has_notiz ? "currentColor" : "none"}
            />
          </div>
